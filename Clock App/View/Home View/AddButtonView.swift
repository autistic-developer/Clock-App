...
struct AddButtonView: View {
    ...
    var body: some View {
        addButoom
            ...
            .onChange(of: scenePhase) { newValue in
                if newValue == .active{
                    alarmManager.setIsGrantedStatus()
                }
            }
    ...
    var sheetView: some View{
        VStack{
            UIDatePickerView(date: $date)
            HStack{
                
                sheetButton(text:"Cancel") {
                    showTimePicker.toggle()
                }
                
                sheetButton(text:"Add"){
                    let _ = Reminder.init(context: viewContext, date: date)
        ...
        .presentationDetents([.height(300.w)])
        
        
    }
    private func sheetButton(text:String, action:  @escaping () -> Void)-> some View{
    ...
    private var addButoom:some View{
        layer1(shape: Circle())
            .frame(width: 78.w, height: 78.w)
            .overlay{
                Button(role: nil){
                    if alarmManager.isGranted{
                        showTimePicker.toggle()
                    }
                    else{
                        Task{
...
struct UIDatePickerView : UIViewRepresentable{
    ...
    func makeUIView(context: Context) -> UIDatePicker {
        ...
        view.addTarget(context.coordinator, action: #selector(context.coordinator.valueChanged(_:)), for: .allEvents)
        return view
    }
    ...
    func makeCoordinator() -> UIDatePickerView.Coordinator  {
